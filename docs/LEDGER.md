...
| Open-ended | **WHICH PREMIER LEAGUE CLUBS DOES THE GROUP SUPPORT?** A reader asked for football in Sports on 2026-08-06 and specified the shape himself: *"emphasis on news from the teams we like, general from the rest of the league."* The machinery shipped the same day, but `config.PREMIER_LEAGUE_FOLLOWED_CLUBS` is **empty**, so the paper runs generic league coverage — correct, and not what was asked for. **Do not guess an allegiance:** printing a rival's result as "our" news is the error a supporter never forgets. Fill the list in `config.py` the moment it is known and the emphasis turns on with no other change. `_football_advisory()` nudges about this in season | **OPEN — waiting on the group** |
