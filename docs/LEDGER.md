...
| Due | What | State |
|---|---|---|
| **2026-11-01** | **Switch the routine's cron to `15 11 * * *`.** EDT ends at 2 AM that Sunday. The cron is UTC-fixed, so `15 10 * * *` becomes a **5:15 AM ET** wake and a paper that posts an hour early, on the first day of a basho month. `config.POST_CRON_UTC_STANDARD` is the value; `config.cron_for(date)` says which one belongs on any date. Note `--not-before 07:00` limits the damage — it holds delivery at 7:00 either way — but the routine would sit idle for an hour burning its head start | **OPEN** |
| **2027-03-14** | **Switch the routine's cron back to `15 10 * * *`.** EDT resumes at 2 AM that Sunday; leaving the winter cron installed wakes the routine at 7:15 ET, which is already past the delivery target and *after* the forecast the paper points at | **OPEN** |
