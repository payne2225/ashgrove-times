...
| Open-ended | **The Pages build runs longer than the 120s poll allows.** 2026-08-06: `editions/2026-08-06.html` 404'd for the full 120s window so the link was omitted, but it returned 200 about 10–12 minutes after the push, and the root and archive pages had rebuilt by then. **Pages is enabled and healthy — this is build lag, not an outage, so do not set `PAGES_ENABLED = False`.** The permalink is reachable from the archive regardless. If it recurs, the fix is to push `site/` immediately after render and do the ledger work while the build runs, not to poll longer and post late | **OPEN** |
